...
def is_valid_bst(root: TreeNode) -> bool:
    def is_valid_bst_bounded(node: TreeNode,
                             lower: Optional[int],
        ...
        if higher is not None and node.val >= higher:
            return False
        left_is_valid = (not node.left
                         or is_valid_bst_bounded(node.left, lower, node.val))
        right_is_valid = (not node.right
                          or is_valid_bst_bounded(node.right, node.val, higher))
